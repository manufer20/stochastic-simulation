...
# ------------------------------------------------------------------
#  Bed‑split scenarios used in run_split_sensitivity(…)
#  • Keys are labels; values can be absolute bed counts
#    ‑ OR ‑ fractions that sum to 1 (they will be scaled to total_beds)
# ------------------------------------------------------------------
BED_SCENARIOS = {
...
def run_split_sensitivity(scenarios, reps=30, total_beds=165):
    ...
    for label, frac in scenarios.items():
        ...
        totals = []
        for _ in range(reps):
            _, _, _, _, pen, _ = simulate_hospital_flow(beds_override=beds)
